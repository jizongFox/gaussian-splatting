        ...
        for config in validation_configs:
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx, viewpoint in enumerate(config["cameras"]):
                    ...
                    image = torch.clamp(
                        renderFunc(viewpoint, model=scene.gaussians, **renderArgs)[
                            "render"
                        ],
                        0.0,
                        1.0,
                    ...
                    l1_test += (
                        l1_loss(image * mask_torch, gt_image * mask_torch)
                        .mean()
                        .double()
                    )
                    psnr_test += (
                        psnr(image * mask_torch, gt_image * mask_torch).mean().double()
                    )
